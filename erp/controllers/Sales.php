    ...
    function add($sale_order_id = NULL, $delivery_id = NULL,$quote_ID = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
			...
			if($invoice_view == 0){
                ...
                //redirect("sales/invoice_eang_tay_a5/".$s->row()->id);
                //redirect("sales/print_st_invoice/" . $s->row()->id);
                redirect("sales/knk_invoice/" . $s->row()->id);
...
function invoice_concrete_angkor($id=null)
{
    ...
    function knk_invoice_landscape($id = null)
    {
        $this->erp->checkPermissions('add', true, 'sales');
        ...
        $this->data['title'] = "2";
        $this->data['sid'] = $id;
        $this->load->view($this->theme . 'sales/knk_invoice_lancape', $this->data);
    }
